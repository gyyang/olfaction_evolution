...
def _analyze_perturb(n_pn=N_PN, n_kc=N_KC, n_kc_claw=N_KC_CLAW,
    ...
    if x_dist == 'uniform':
        ...
        # X = torch.from_numpy(np.random.rand(n_pts, n_pn)).float().to(device)
    elif x_dist == 'gaussian':
        X = torch.randn(n_pts, n_pn, device=device) * 0.5 + 0.5
        # X = torch.from_numpy(np.random.randn(n_pts, n_pn)*0.5+0.5).float().to(device)
    else:
        raise NotImplementedError()
    ...
    if normalize_x:
        raise NotImplementedError
        X = normalize(X)
    print('get X', time.time() - start_time)
    start_time = time.time()
...
def analyze_perturb(n_kc_claw=N_KC_CLAW, n_rep=1, **kwargs):
    ...
    for i in range(n_rep):
        ...
        print('torch cat', time.time() - start_time)
        X.append(X0)
        Y.append(Y0)
        Y2.append(Y20)
...
def simulation(K, compute_dimension=False, **kwargs):
    ...
    print('analyze perturb torch', time.time() - start_time)
    start_time = time.time()
    Y, Y2 = set_coding_level(Y, Y2, **kwargs)
    print('set coding level', time.time() - start_time)
    start_time = time.time()
...
if __name__ == '__main__':
    ...
    for i in range(10):
        print(i)
        start_time = time.time()
        res = simulation(K=10, compute_dimension=True, **params)
